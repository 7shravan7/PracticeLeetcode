...
/* **Easy**      811. Subdomain Visit Count
 * 
 * A website domain like "discuss.leetcode.com" consists of various subdomains. At the top level, we have "com", at 
 * the next level, we have "leetcode.com", and at the lowest level, "discuss.leetcode.com". 
...
public class SubdomainVisitCount {
	...
	public List<String> subdomainVisits(String[] cpdomains) {
		...
		for(String cpDomain : cpdomains) {
			String[] domainSplitString = cpDomain.split(" ");
			int countPair = Integer.valueOf(domainSplitString[0]);
			String fullDomain = domainSplitString[1];
			int domainCount = domainCountMap.getOrDefault(fullDomain, 0);
			domainCountMap.put(fullDomain, domainCount+countPair);
			int dotIndex = fullDomain.indexOf('.');
			while(dotIndex!=-1) {
				String innerDomain = fullDomain.substring(dotIndex+1);
				int innerDomainCount = domainCountMap.getOrDefault(innerDomain, 0);
				domainCountMap.put(innerDomain, innerDomainCount+countPair);
				dotIndex = innerDomain.indexOf('.');
				fullDomain = innerDomain;
			}
		}
		domainCountMap.entrySet().forEach(entry->{
			resultlist.add(new StringBuilder(String.valueOf(entry.getValue())).append(" ").append(entry.getKey())
					.toString());
		});
		return resultlist;
    }
	...
	public static void main(String[] args) {
		...
		List<String> resultList1 = visitCount.subdomainVisits(cpdomains1);
		printList(resultList1);
		String[] cpdomains2 = {"900 google.mail.com", "50 yahoo.com", "1 intel.mail.com", "5 wiki.org"};
		List<String> resultList2 = visitCount.subdomainVisits(cpdomains2);
